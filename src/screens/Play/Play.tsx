    ...
    firestore()
      ...
      .then((snap) => {
        if (snap.exists) {
          ...
          if (data?.score < score) {
            firestore().collection('leaderboard').doc(uid).update({
              score,
            });
          }
          return;
        } else {
          firestore().collection('leaderboard').doc(uid).set({
  ...
  return (
    <View style={styles.container}>
      ...
      <GameEngine
        ...
        running={running}
      />
      <Text style={[styles.score, Styles.fontExtraLarge]}>{score}</Text>
      {!running && (
